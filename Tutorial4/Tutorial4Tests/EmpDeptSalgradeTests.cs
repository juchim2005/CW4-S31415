...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        var chicago = depts.Where(d => d.Loc == "CHICAGO").Select(d => d.DeptNo).ToList();
        List<Emp> result = emps.Where(e => chicago.Contains(e.DeptNo)).ToList();; 
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var deptAve = emps.GroupBy(e => e.DeptNo).ToDictionary(g => g.Key, g => g.Average(e => e.Sal));

        var result = emps.Where(e => e.Sal > deptAve[e.DeptNo]).Select(e => e.EName).ToList();
